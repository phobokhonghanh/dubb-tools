...
from core.use_cases.merger_service import MergerCallbacks, MergerService
...
from infrastructure.providers.translator import parse_srt, replace_all, serialize_srt
from utils.video_merger import require_ffmpeg
from utils.video_splitter import get_model_paths, split_video_audio
from infrastructure.database.checkpoint import get_completed_steps, save_step_checkpoint
...
def run_pipeline(
    ...
    *,
    ...
    try:
        ...
        completed_steps = get_completed_steps(context.job_id)
        ...
        for index, step_status in enumerate(steps, start=1):
            ...
            # Kiểm tra checkpoint SQLite để skip bước nếu đã chạy thành công trước đó
            can_skip = False
            if step in completed_steps:
                chk = completed_steps[step]
                if chk.get("status") == "success":
                    out_path = chk.get("output_path")
                    if out_path and Path(out_path).exists():
                        if step == "split":
                            vocals_path = Path(context.job_dir) / f"{context.source_stem}_vocals.wav"
                            background_path = Path(context.job_dir) / f"{context.source_stem}_background.wav"
            ...
            if can_skip:
                _log(on_log, f"Bước {STEP_TITLES.get(step, step)} đã chạy thành công trước đó. Đang khôi phục từ checkpoint...")
                out_path = completed_steps[step]["output_path"]
                if step == "download":
                    context.downloaded_video = out_path
                    context.local_video = out_path
                elif step == "split":
                    context.muted_video = out_path
                    context.vocals_audio = str(Path(context.job_dir) / f"{context.source_stem}_vocals.wav")
                    context.background_audio = str(Path(context.job_dir) / f"{context.source_stem}_background.wav")
                elif step == "stt":
                    context.transcript_srt = out_path
                elif step == "translate":
                    context.translated_srt = out_path
                elif step == "tts":
                    context.speech_audio = out_path
                elif step == "merge":
                    context.final_video = out_path

                step_status.output_path = out_path
                step_status.state = "done"
                step_status.percent = 100
                step_status.message = f"Khôi phục hoàn tất {STEP_TITLES.get(step, step)} từ checkpoint."
                if on_step_done:
